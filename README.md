...
| `check_data_integrity` | Pool-to-NBT-to-worldgen-to-structure-set reference chain: all NBT files referenced by template pools exist; all worldgen structures reference real files; structure sets reference real structures. Also the reverse — every `worldgen/structure/*.json` must be placed by something, either an entry in a `structure_set` (any namespace, so a pack overriding a vanilla set counts) or an MSL `replace_vanilla` `replacement_structure`. A structure nothing places can never generate and fails silently. Handles `moogs_structures:versioned_single_pool_element` pool elements. | Fails |
...
| `check_registries` | Block and item IDs in loot tables and NBT palettes exist in the MC registries for the targeted versions. The two halves are independent — a pack with no `loot_table` directory still has its palettes scanned. Validates palette entries against the per-file minimum version (derived from versioned pool elements); the game re-maps renamed blocks upward through DataFixerUpper on load, so the floor is the only version that can fail. An unknown `minecraft:` block is annotated with the first release after that floor that has it (every stable release in `misode/mcmeta` is probed, so the note stays right as new versions ship), or `unknown ID`. Files in the multi-variant `palettes` form are scanned too, every variant. | Fails |
| `check_worldgen_schemas` | Template pool, structure, structure set, and processor list JSON files validate against bundled MC + MSL schemas. Unknown `moogs_structures:*` type ids are rejected (typo catcher). MSL types with per-MC-version registration windows (e.g. `waterlogging_fix_processor` on 1.20 only, `trial_spawner_randomizing_processor` on 1.21+) are only flagged when they're dead on **every** targeted MC version. The `advanced_random_spread` schema covers the optional `when_replacing` block (MSL 3.1.3+): `modid`, `vanilla_key`, `spacing` and `separation` are all required, and any other key inside it is an error. Also catches two `y_allowance` shapes JSON schema cannot express: `max_y_allowed` below `min_y_allowed` (MSL rejects it at datapack load), and `max_y_allowed` set with no `min_y_allowed` on the generic jigsaw type (MSL unwraps the absent `min` and crashes chunk generation). *(MSL version-gated in v1.8.3; MSL 3.1.0 types added in v1.9.0; y_allowance crash in v1.10.2)* | Fails |
...
| `check_block_entity_components` | The block-entity `components` key arrived at 1.20.5, so a file whose minimum covered version is below 1.20.5 must not carry one on any block entity. Generalises a rule `check_sign_nbt` previously applied to signs only. Gated on the file's wired target range, not its stored `DataVersion`. The inverse is deliberately not checked: vanilla omits the key on 4832 of 4848 block entities in its own structure files (1.21.5 and 1.21.10 alike), and mixes both shapes within a single file, so absence at 1.20.5+ is normal rather than an error. | Fails |
...
| `check_jigsaw_pools` | Every jigsaw block's `pool` field in non-orphaned NBT files references a real template pool. Own-namespace refs must have a matching `.json` in `worldgen/template_pool/`; `minecraft:*` refs must exist in vanilla on at least one targeted version (mcmeta's `worldgen/template_pool` registry; taken as valid when it cannot be fetched); any other namespace warns. *(new in v1.5.0; vanilla pools verified in v1.11.0)* | Warn only |
...
| `check_entity_nbt_keys` | Per-mob version-gated key validation. Checks that version-restricted NBT keys only appear in files targeting the right MC version range. Table covers: `minecraft:painting` (`Motive` was renamed to `variant` in 1.19, so it is dead on every supported version; `variant` is right everywhere) and `minecraft:wolf` (`variant` is 1.20.5+ only). Only `minecraft:*` entities are checked. Table lives in `data/entity_nbt_keys.json` and can be extended without code changes. *(new in v1.6.0)* | Fails |
...
| `check_merchant_offers` | Wandering traders, and villagers that already have a profession, placed by a structure should carry saved `Offers`. Without them the trades are rolled the first time the entity is read, which Moog's Structure Lib before 3.3.1 did during world generation; a map trade rolled there can freeze the server. Riders are walked; spawner-nested entities are skipped. Orphaned NBT files are excluded. | Warns; never fails |
...
Everything the checks share is read once. `core/` parses every structure file
once (`core/nbt.py`, a parser built for structure files: plain blocks stay in
two flat int arrays, only block entities, entities and the palette become
dicts), indexes it once (block entities, every entity including riders and
spawner-nested ones, blocks by palette index), reads every JSON file once, and
fetches every mcmeta registry once. The 29 checks then apply their rules to
those indexes. On Moog's Voyager Structures (355 files, 1.27 million blocks)
the check phase takes about 2 seconds.

Version data is never hard-coded. DataVersions, registries, biome tags and the
vanilla structure and template-pool lists all come from
[`misode/mcmeta`](https://github.com/misode/mcmeta), pinned per Minecraft
version and cached under `cache/`; a new release needs only its name in
`mc_versions`.
