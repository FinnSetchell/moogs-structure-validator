...
| `check_block_entity_components` | Every block entity gained a `components` map at 1.20.5 (usually empty), and it is never written before. So a file whose minimum target is 1.20.5 or later must have the key on every block entity, and a file below that must not have it on any. Gated on the file's wired target range, not its stored `DataVersion`. | Fails |
