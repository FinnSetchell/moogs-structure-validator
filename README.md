...
| `check_block_entity_components` | The block-entity `components` key arrived at 1.20.5 and is never written before it, so a file floored below 1.20.5 must not carry one on any block entity — ERROR. This generalises a rule `check_sign_nbt` previously applied to signs only. The inverse is not enforced: above 1.20.5 the key is a per-file property (whole files carry it on every block entity or on none, same types and same `DataVersion` on both sides), so its absence tracks how the file was written rather than an authoring mistake. A file that disagrees with *itself* — some block entities carrying the key, others not — is a WARN. Gated on the file's wired target range, not its stored `DataVersion`. | Fails on ERROR; WARN prints only |
