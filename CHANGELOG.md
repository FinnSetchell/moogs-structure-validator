...
### Added
- **`check_merchant_offers` (new, warn-only).** A wandering trader, or a villager that already has a profession, placed by a structure without saved `Offers` rolls its trades the first time anything reads them. Before Moog's Structure Lib 3.3.1 that happened during world generation, where an explorer-map trade searches for a structure off the server thread and freezes the server. The check warns on such merchants, riders included; spawner-nested entities spawn later on the server thread and are skipped. It never fails a build. On the fleet it warns on Mineshafts Reimagined, Ocean Structures and Voyager Structures, all of which need MSL 3.3.1 or newer.
- **MSL 3.3.0 replacement options and `when_replacing`.** `check_msl_replace_vanilla` checks a replacement's optional `options` block: only `alias_lookups`, `inherit_spawn_overrides`, `redirect_locate` and `mirror_tags` are allowed, each a boolean. MSL ignores an unknown key and keeps the default of on, so a misspelled switch silently stays on, and it reads a non-boolean through `parseBoolean`, so `1` or `"yes"` quietly means off. Both are errors. The `advanced_random_spread` schema covers the optional `when_replacing` block (all four fields required), and `check_msl_placements_and_processors` requires `spacing` > `separation` in the base placement and in `when_replacing` (MSL throws at load otherwise), gives `when_replacing` the same preset check as the other MSL placements, and warns when a set places a replacement structure with no `when_replacing` at all, since it then keeps its own density while standing in for vanilla. On the fleet that warning fires three times on Temples Reimagined (desert temple, jungle temple, ocean temple); no verdict changes.
